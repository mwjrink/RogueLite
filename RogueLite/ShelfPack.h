...
namespace mapbox
{
    ...
    class Bin
    {
      ...
      public:
         ...
         * Bin b(-1, 12, 16);
         */
        explicit Bin(int32_t id1 = -1, int32_t w1 = -1, int32_t h1 = -1, int32_t maxw1 = -1, int32_t maxh1 = -1,
                     int32_t x1 = -1, int32_t y1 = -1)
    ...
    class ShelfPack
    {
      public:
         ...
         * @param   {bool} [options.inPlace=false] If `true`, the supplied bin objects will be updated inplace with `x` and
         * `y` values
        ...
        void shrink()
        {
            if (shelves_.size())
            {
                ...
                {
                    h2 += shelf.h();
                    w2 = (shelf.w() - shelf.wfree() > w2) ? shelf.w() - shelf.wfree() : w2;
                }
