...
#include "Libraries/llvm/ErrorHandling.h"
#include <cassert>
#include <cstdlib>
#include <mutex>
#include <new>
...
void llvm::report_fatal_error(const char* Reason, bool GenCrashDiag) { report_fatal_error(Twine(Reason), GenCrashDiag); }
...
void llvm::report_fatal_error(StringRef Reason, bool GenCrashDiag) { report_fatal_error(Twine(Reason), GenCrashDiag); }
...
static void out_of_memory_new_handler() { llvm::report_bad_alloc_error("Allocation failed"); }
...
{
  public:
    NewHandlerInstaller() { install_out_of_memory_new_handler(); }
} new_handler_installer;
#endif

void llvm::llvm_unreachable_internal(const char* msg, const char* file, unsigned line)
{
...
{
    LLVMFatalErrorHandler handler = LLVM_EXTENSION reinterpret_cast<LLVMFatalErrorHandler>(user_data);
    handler(reason.c_str());
}

void LLVMInstallFatalErrorHandler(LLVMFatalErrorHandler Handler)
{
    install_fatal_error_handler(bindingsErrorHandler, LLVM_EXTENSION reinterpret_cast<void*>(Handler));
}
...
#define MAP_ERR_TO_COND(x, y) \
    case x: return make_error_code(errc::y)
...
std::error_code llvm::mapWindowsError(unsigned EV)
{
    switch (EV)
    {
        ...
        default: return std::error_code(EV, std::system_category());
    }
}
